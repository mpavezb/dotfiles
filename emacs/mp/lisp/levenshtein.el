...
(defun levenshtein-distance (str1 str2) 
  ...
  (let* ((make-table ;; Multi-dimensional array object.
          (function (lambda (columns rows init) 
		      (make-vector rows (make-vector columns init))))) 
	 (tref ;; Table access method.
          (function (lambda (table x y) 
		      (aref (aref table y) x)))) 
	 (tset ;; Table write method.
          (function (lambda (table x y object) 
	 ...
	 (length-str2 (length str2))
         ...
         (d (funcall make-table (1+ length-str1) 
		     (1+ length-str2) 0))) ;; Initialize to zero.
    ...
    (let ((i 1)) 
	  ...
	  (while (<= j length-str2) ;; for j from 1 to lenStr2
            (let* ((cost
                   ...
                   (substitution (+ (funcall tref d (1- i) 
					     (1- j)) cost))) 
